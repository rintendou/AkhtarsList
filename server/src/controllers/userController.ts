...
export const getUser = async (req: Request, res: Response) => {
    ...
    if (!userId) {
        return res.status(400).json({ message: 'userId params is required!', data: null, ok: false })
    }
    ...
    try {
        ...
        const existingUser = await UserModel.findById(userId).populate('listedListings')
        if (!existingUser) {
            return res.status(404).json({ message: 'User not found!', data: null, ok: false })
        }
...
export const withdrawFunds = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (withdrawAmount > user.balance) {
            return res.status(404).json({ message: 'Insufficient Funds!', data: null, ok: false })
        }
