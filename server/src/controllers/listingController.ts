...
export const fetchUserListings = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Check if listed listings field is empty
    const listedListings = existingUser.listedListings
    if (listedListings.length === 0) {
      return res
        .status(404)
        .json({ message: "No listings found!", data: null, ok: false })
    }
